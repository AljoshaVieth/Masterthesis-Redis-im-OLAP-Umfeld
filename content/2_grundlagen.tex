...
\subsection{Feldtypen und Optionen}
RediSearch bietet eine Auswahl an Feldtypen (\emph{field types}) für verschiedene Indexierungszwecke~\cite{redis_ltd_field_nodate}.

\textbf{\emph{Number fields}} speichern zählbare Werte in Ganzzahl- oder Gleitkommazahlen und erlauben sortierbare Bereichsabfragen.

\textbf{\emph{Geo fields}} sind für geografische Koordinaten vorgesehen, die Radiusabfragen für standortbezogene Suchfunktionen ermöglichen.

\textbf{\emph{Geoshape fields}} ermöglichen Polygonabfragen mit WKT-Notationen (\enquote{well-known text}) und unterstützen sowohl geografische (\emph{spherical}) als auch kartesische (\emph{flat}) Koordinaten. Die Koordinaten bestehen aus zwei Werten die die Kanten des Polygon angeben~\cite{redis_ltd_ftcreate_nodate}.

\textbf{\emph{Vector fields}} halten Gleitkommavektoren, die diverse unstrukturierte Daten repräsentieren können, und unterstützen Vektorsimilaritätssuchen. Sie werden oft für externe Machine-Learning Modelle genutzt.

\textbf{\emph{Tag fields}} speichern Textdaten als Tags oder Labels, die ohne Tokenisierung verarbeitet werden, und eignen sich zur Datenorganisation und -kategorisierung.

\textbf{\emph{Text fields}} sind für die Speicherung von menschlicher Sprache konzipiert, mit Funktionen wie Kleinbuchstaben-Umwandlung, Tokenisierung und gewichteter Indexierung für effiziente Volltextsuche.
