...
\section{\acf{OLAP}}
Kleppmann~\cite[S.~97f]{kleppmann_datenintensive_2019} beschreibt OLTP und OLAP wie folgt:
Viele Unternehmen nutzen heute große Mengen an Geschäftsdaten für die strategische Planung.
Aus der Analyse dieser Daten können Handlungsempfehlungen abgeleitet werden.
Typische Analysen sind z.B. die Berechnung des Umsatzes einer Filiale im letzten Jahr im Vergleich zu den Vorjahren oder die Analyse des Absatzes einzelner Produkte in einem bestimmten Quartal.
Diese Analysen erfordern eine große Menge an Daten, einschließlich historischer Daten.
Klassische Datenbanksysteme für Transaktionen, z.B. in einem Online-Shop, auch \acf{OLTP} genannt, sind für viele Schreib- und Lesezugriffe optimiert, verwenden meist wenig komplexe Abfragen (Queries) und enthalten häufig keine historischen Daten.
Datenbanken für analytische Zwecke sollten dagegen stark auf Lesezugriffe optimiert sein und müssen komplexere Abfragen in angemessener Zeit ausführen können.
Bei diesem Einsatzgebiet von Datenbanken spricht man vom.
...
Die Informationen in diesem Abschnitt basieren auf dem Werk von Vaisman und Zimányi~\cite[S.~3f]{vaisman_data_2022}.
Data Warehouses wurden entwickelt, um Daten zu speichern, die für strategische Entscheidungen im Geschäftsumfeld nützlich sein können.
...
Um Entscheidungen aus den Daten ableiten zu können, sind sowohl historische Daten als auch komplexe Abfragen notwendig.
Um diese Probleme zu lösen, enthält ein Data Warehouse historische Daten und bietet ein optimiertes Datenmodell für komplexere Abfragen.

In der ursprünglichen Definition werden Data Warehouses als Sammlung von subjektorientierten, integrierten, nicht flüchtigen und zeitvariablen Daten beschrieben. 
...
Mit integrierten Daten ist gemeint, dass Daten aus verschiedenen Umgebungen in dem Data Warehouse integriert sind.
...
Zeitvariabel beschriebt daher, dass Daten aus verschiedenen Zeitpunkten gespeichert werden.
...
Ein Ort kann etwa aus dem Ortsnamen, dem Bundesland, dem Land und dem Kontinent bestehen.
...
Die folgenden Abschnitte geben eine Übersicht über den \emph{TPC-H} und den \emph{Star Schema Benchmark}.
\subsection{\acf{TPC-H}}
Der \acf{TPC-H} wurde zum Testen von analytischen Datenbanken entwickelt.
Die Daten und Abfragen des Benchmarks sind auf eine Datenbank ausgelegt, mit der ein Anbieter seine Geschäftsdaten analysieren kann~\cite[S.~624-626]{barata_overview_2015}. Er dient außerdem als Grundlage für den \emph{Star Schema Benchmark}, wird selbst aber hier nicht weiter erläutert.

\subsection{\acf{SSB}}
Der \acf{SSB} ist eine abgewandelte Form des \ac{TPC-H}.

\subsubsection{Schema des \ac{SSB}} 
Der \ac{SSB} verwendet die gleichen Daten, nutzt dafür jedoch ein Sternschema statt eines normalisierten Schemas.
...
\end{lstlisting}
Alle Queries sind in \cref{sec:appendix-ssb-queries} aufgelistet.
...
Redis ist ein Key-Value Store, der vollständig im Arbeitsspeicher (in-memory) arbeitet und häufig als Datenbank, Cache, Message Broker oder Streaming Engine eingesetzt wird~\cite{redis_ltd_introduction_nodate}.
...
Mit Erweiterungen, so genannten Modulen, können weitere Datentypen wie z.~B. JSON oder Timeseries unterstützt werden.
Redis wird von vielen bekannten Unternehmen wie GitHub, Stack Overflow, Snapchat, Craigslist oder X verwendet~\cite{redis_ltd_whos_nodate}.

Mit \enquote{Auto Tiering} besteht die Möglichkeit, Arbeitsspeicher zusammen mit SSD-Speicher zu nutzen, was die Infrastrukturkosten um bis zu 70\% senken kann~\cite{redis_ltd_auto_nodate}. Dieses Feature ist jedoch nur mit dem kostenpflichtigen \enquote{Redis Enterprise} nutzbar.
...
\emph{Redis Stack} kombiniert Redis mit den Modulen \emph{RediSearch}, \emph{RedisJSON}, \emph{RedisTimeSeries} und \emph{RedisBloom} zu einem Paket.
...
In den folgenden Abschnitten werden die verschiedenen Datentypen von Redis beschrieben. Die Informationen stammen dabei aus der Redis Dokumentation. Die einzelnen Seiten sind jeweils am Ende der Abschnitte als Quelle angegeben.
...
Mit Redis-Clustern kann Redis horizontal skaliert werden, wobei die Datenverteilung automatisch über mehrere Knoten erfolgt. Laut der Dokumentation von Redis~\cite{redis_ltd_scale_nodate} zeichnet sich diese Architektur durch ihre Fähigkeit aus, auch in gewissen Ausfallszenarien weiterhin funktionsfähig zu bleiben. Beispielsweise kann der Cluster Operationen fortsetzen, selbst wenn einzelne Knoten ausfallen oder Kommunikationsprobleme innerhalb des Clusters auftreten. Es ist jedoch zu beachten, dass bei umfangreicheren Ausfällen, wie dem Ausfall der Mehrheit der Master-Knoten, der gesamte Cluster nicht mehr verfügbar sein wird.

Der Redis-Cluster garantiert keine strenge Konsistenz. Schreiboperationen, die dem Client als erfolgreich gemeldet wurden, können unter bestimmten Umständen verloren gehen. Der Hauptgrund dafür ist die asynchrone Replikation: Ein Master-Knoten bestätigt Schreiboperationen, bevor er sie an seine Replikate weiterleitet. Fällt dieser Master aus, bevor die Replikate aktualisiert wurden, kann ein Replikat ohne diese Schreibvorgänge zum neuen Master werden, wodurch die Daten verloren gehen.

Redis Cluster unterstützen synchrone Schreibvorgänge über den \emph{WAIT}-Befehl, was den Datenverlust reduziert, aber auch dann keine hohe Konsistenz bietet. Bei komplexeren Ausfallszenarien ist es immer noch möglich, dass ein Replikat, das den Schreibvorgang nicht empfangen konnte, zum Master gewählt wird. Bei Netzwerkpartitionen können Schreibvorgänge ebenfalls verloren gehen, wenn ein isolierter Client weiterhin auf einen Master schreibt, der später durch ein Replikat auf der Mehrheitsseite der Partition ersetzt wird.



\subsection{RedisInsight}
RedisInsight ist eine kostenlos GUI-Anwendung zur Verwaltung von Redis-Datenbanken \cite{redis_ltd_redisinsight_nodate}. Sie bietet unter anderem eine visuelle Ansicht der Daten, eine integrierte CLI sowie Analysen der Datenbank. Im Rahmen dieser Arbeit wurde sie für manuelle Anfragen und zur Visualisierung genutzt.
\section{RediSearch}
...
- Suche in allen Textattributen:
 \lstinline|FT.SEARCH recipeIndex "example text"|
...
\subsection{Aggregations}\label{sec:redisearch-aggregations}
...
Anschließend werden die die beiden Felder \lstinline|@name @cooking_time| mit\\ \lstinline|LOAD 2 @name @cooking_time|  geladen. Für die restliche Pipeline ist dies nicht zwingend notwendig, da \lstinline|SORTBY| auch bei nicht geladenen Feldern funktioniert, solange diese \emph{sortable} sind (siehe \ref{sec:redisearch-aggregation-syntax}). Jedoch würden ohne diese Anweisung die Namen der Rezepte nicht im Ergebnis der Abfrage zurückgegeben werden.\\
Als letzter Schritt werden die Ergebnisse anhand der Kochzeit sortiert: \lstinline|SORTBY 2 @cooking_time DESC MAX 3|. \lstinline|DESC| gibt hierbei an, dass die Ergebnisse absteigend sortiert werden. Die Anweisung \lstinline|MAX 3| sorgt dafür, dass nur die ersten drei Dokumente zurückgegeben werden.
...
Außerdem wird mit \lstinline|COUNT 0 AS num_recipes| ein neues Feld namens \enquote{\emph{num\_recipes}} hinzugefügt, das angibt, wie viele Rezepte in jeder Schwierigkeitskategorie vorhanden sind.
...
Anschließend werden die Dokumente nach Schwierigkeitsgrad gruppiert und ein neues Feld \emph{avg\_cooking\_time} eingefügt, das die berechnete durchschnittliche Kochzeit der Rezepte nach Kategorie beinhaltet.\\
Anschließend werden die Kategorien mit \lstinline|SORTBY 2 @avg_cooking_time ASC| nach durchschnittlicher Kochzeit sortiert.
...
\subsection{RediSearch in einem Cluster}
RediSearch ermöglicht Clustering ausschließlich innerhalb der kostenpflichtigen \emph{Redis Cloud} und \emph{Redis Enterprise Software}~\cite{redis_ltd_redisearch_2023}, wobei letztere laut Produktseite ~\cite{redis_ltd_scale_nodate} eine nahtlose Skalierung von Suche und Abfragen über einen gesamten Cluster hinweg ermöglicht, um Indizes auf Milliarden von Dokumenten über Hunderte von Servern zu erweitern.
...
\subsection{Lua in Redis}\label{sec:lua-in-redis}
